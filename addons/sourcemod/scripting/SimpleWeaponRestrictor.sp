...
public void OnPluginStart() {
	...
	RegAdminCmd("wr_unrestrict", Command_Unrestrict, ACCESS_FLAG, "Unrestricts a weapon.");
	RegAdminCmd("wr_reload", Command_Reload, ACCESS_FLAG, "Reloads the configuration file.");
	...
	AutoExecConfig(true, "moongetsu/weapon_restrict");
...
void LoadConfigFile(bool late){
	if(g_bDebug)
		PrintToServer("[HardGame] Loading configuration file");
	...
	if(!FileExists(path)){
		if(g_bDebug)
			PrintToServer("[HardGame] File not found...");
		return;
	}
	...
	if(config == null){
		PrintToServer("[HardGame] Could not open configuration file...");
		return;
	}
	...
	while(config.ReadLine(weapon, sizeof(weapon))){
		...
		if(g_bDebug)
			PrintToServer("[HardGame] Config file: processing line %d : '%s'", line, weapon);
		
		// If the string is empty: skip.
		if(strlen(weapon) < 1){
			if(g_bDebug)
				PrintToServer("[HardGame] Config file: Line %d is empty!", line);
			continue;
		}
		...
		if(!IsWeaponAliasValid(weapon)){
			PrintToServer("[HardGame] Config file: WARNING - Invalid entry: %s! (Line: %d)", weapon, line);
			continue;
		}
		
		// Check if already restricted
		if(g_alRestrictedWeapons.FindString(weapon) != -1){
			PrintToServer("[HardGame] Config file: WARNING - Duplicate entry: %s! (Line: %d)", weapon, line);
			continue;
		}
		...
		if(g_bDebug)
			PrintToServer("[HardGame] Config file: %s is restricted!", weapon);
		
		// If this load was done late, check all clients for the given weapon.
		if(late){
			if(g_bDebug)
				PrintToServer("[HardGame] Config file: Removing %s from all players.", weapon);
			RemoveWeaponFromAllPlayer(weapon);
		}
	}
	
	delete config;
	PrintToServer("[HardGame] Config file: Loaded %d restrictions.", restrictions);
	if(g_bDebug)
		PrintToServer("[HardGame] Configuration file has been loaded");
}
...
public Action Event_WeaponEquip(int client, int weapon){
	...
	if(g_alRestrictedWeapons.FindString(weaponClass) != -1){
		// Notify the client
		CPrintToChat(client, "[{green}HardGame{default}] {green}%s {default} is restricted and cannot be picked up.", weaponClass);
		// Kill the bugger
...
public Action CS_OnBuyCommand(int client, const char[] weapon){
	if(g_alRestrictedWeapons.FindString(weapon) != -1){
		CPrintToChat(client, "[{green}HardGame{default}] {default}You cannot buy {green}%s{default}, as it is restricted.", weapon);
		return Plugin_Handled;
	}
	return Plugin_Continue;
}
...
public Action Command_Reload(int client, int args){
	if(g_bDebug)
		PrintToServer("[HardGame] Reloading configuration file");
	g_alRestrictedWeapons.Clear();
	LoadConfigFile(true);
	CReplyToCommand(client, "[{green}HardGame{default}] {default}Configuration file has been reloaded!");
}

public Action Command_Restrict(int client, int args){
	if(args < 1){
		CReplyToCommand(client, "[{green}HardGame{default}] {default}Command: sm_restrict <weapon name (e.g. awp)>");
		return Plugin_Handled;
	}
	...
	if(!IsWeaponAliasValid(weaponAlias)){
		CReplyToCommand(client, "[{green}HardGame{default}] {green}%s {default} is an invalid weapon!", weaponAlias);
		return Plugin_Handled;
	}
	
	// Check if already restricted
	if(g_alRestrictedWeapons.FindString(weaponAlias) != -1){
		CReplyToCommand(client, "[{green}HardGame{default}] {green}%s {default}is already restricted!", weaponAlias);
		return Plugin_Handled;
	}
	
	g_alRestrictedWeapons.PushString(weaponAlias);
	
	ShowActivity2(client, "[HardGame] ", "Restricted weapon: %s.", weaponAlias);
	
	CReplyToCommand(client, "[{green}HardGame{default}] {default}Removing {green}%s {default}from all players!", weaponAlias);
...
public Action Command_Unrestrict(int client, int args){
	if(args < 1){
		CReplyToCommand(client, "[{green}HardGame{default}] {default}Command: sm_unrestrict <weapon name (e.g. awp)>");
		return Plugin_Handled;
	}
	...
	if(index == -1){
		CReplyToCommand(client, "[{green}HardGame{default}] {green}%s {default}is invalid or not restricted!", weaponAlias);
		return Plugin_Handled;
	}
	
	g_alRestrictedWeapons.Erase(index);
	
	ShowActivity2(client, "[HardGame] ", "Unrestricted weapon: %s.", weaponAlias);
		...
		for(int slot = CS_SLOT_PRIMARY; slot <= CS_SLOT_C4; ++slot){
			...
			if(StrEqual(weaponClass, entityClass, true)){
				if(RemovePlayerItem(client, weapon)){
					...
					CPrintToChat(client, "[{green}HardGame{default}] {green}%s {default} has been restricted and removed from your inventory.", weaponAlias);
				}
				break;
			}
		}
